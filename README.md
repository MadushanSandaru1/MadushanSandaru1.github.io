...
The HTML includes a generated introduction, CV link, contact information, and metadata so essential content is available before JavaScript loads. After editing JSON, run `node scripts/build.mjs` to update those static fallbacks. Do not edit the generated `STATIC_*` blocks directly.
...
You can open `index.html` directly: file previews use an embedded JSON snapshot generated by `node scripts/build.mjs`. Rebuild after changing content to refresh this snapshot. HTTP/HTTPS previews and the deployed site continue to fetch the JSON files directly. For development, run a local static server:
...
The canonical stylesheet is `scss/main.scss`; it intentionally uses CSS-compatible syntax. The older partials are retained for reference and are not imported. Edit `main.scss`, then run the dependency-free build (Node.js 18+):
...
The build updates `css/main.css` and the static HTML content/metadata from JSON. Commit both generated files when deploying through GitHub Pages. `node scripts/build.mjs --check` checks for stale output without changing files.

## Featured and expanded content

Set `featured: true` on selected projects in `data/projects.json`. An optional `highlight` displays an existing delivery metric prominently. Featured projects appear first by default; All and category filters expose the complete collection.

Courses, technical skills, testimonials, awards, and volunteer entries show a shorter initial selection with accessible View all controls. The limits are in `compactSections()` in `js/renderer.js`; content remains in its original JSON order. No entries have been removed.

## Reliability and accessibility

Essential contact and CV links work without JavaScript. Disabled sections are not fetched, optional data failures show a recovery notice, and theme preferences tolerate blocked browser storage. Dialogs contain keyboard focus and make the background inert. Native anchor navigation preserves URL history and moves focus, and reduced-motion preferences disable animated scrolling and counters.
